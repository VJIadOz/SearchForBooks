...
function InfoBook(props) {
    ...
    function prepareData(resData){
        if(!resData.imageLinks) setdata({image: ""});
        else{
            if(resData.imageLinks.medium) setdata({image:resData.imageLinks.medium});
            if(resData.imageLinks.small) setdata({image:resData.imageLinks.small});
            if(resData.imageLinks.thumbnail) setdata({image:resData.imageLinks.thumbnail});
            if(resData.imageLinks.smallThumbnail) setdata({image:resData.imageLinks.smallThumbnail});
        }
        
        

        // for(let key in resData.imageLinks){
            // switch(resData.imageLinks){
            //     case "medium":
